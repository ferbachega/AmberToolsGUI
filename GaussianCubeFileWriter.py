...
from pCore     import Coordinates3, logFile, LogFileActive, Real1DArray, TextFileWriter, Vector3
from pMolecule import UNITS_LENGTH_BOHRS_TO_ANGSTROMS
...
class RegularCubicGrid3 ( object ):
    ...
    def Summary ( self, log = logFile ):
        ...
        if LogFileActive ( log ):
            ...
            summary.Entry ( "Number of Points", `self.NumberOfPoints ( )`    )
            summary.Entry ( "Grid Spacing",     "%.2f" % ( self.gridspacing, ) )
            summary.Entry ( "Origin x", "%.2f" % ( self.origin[0], ) )
            summary.Entry ( "Extent x", "%.2f" % ( float ( self.npoints[0] ) * self.gridspacing, ) )
            summary.Entry ( "Origin y", "%.2f" % ( self.origin[1], ) )
            summary.Entry ( "Extent y", "%.2f" % ( float ( self.npoints[1] ) * self.gridspacing, ) )
            summary.Entry ( "Origin z", "%.2f" % ( self.origin[2], ) )
            summary.Entry ( "Extent z", "%.2f" % ( float ( self.npoints[2] ) * self.gridspacing, ) )
            summary.Entry ( "Number of x Points", `self.npoints[0]` )
            summary.Entry ( "Number of y Points", `self.npoints[1]` )
            summary.Entry ( "Number of z Points", `self.npoints[2]` )
            summary.Stop ( )
...
class GaussianCubeFileWriter ( TextFileWriter ):
    ...
    def WriteHeader ( self, QORBITALS = False ):
        ...
        # Old code 
        #if QORBITALS: self.file.write ( "%5d%12.6f%12.6f%12.6f\n" % ( - len ( self.system.atoms ), self.grid.origin[0], self.grid.origin[1], self.grid.origin[2] ) )
        #else:         self.file.write ( "%5d%12.6f%12.6f%12.6f\n" % (   len ( self.system.atoms ), self.grid.origin[0], self.grid.origin[1], self.grid.origin[2] ) )
        
        # New code , small changes:  self.system.atoms  - became - self.system.energyModel.qcAtoms.QCAtomSelection ( )
        if QORBITALS: self.file.write ( "%5d%12.6f%12.6f%12.6f\n" % ( - len ( self.system.energyModel.qcAtoms.QCAtomSelection ( )), self.grid.origin[0], self.grid.origin[1], self.grid.origin[2] ) )
        else:         self.file.write ( "%5d%12.6f%12.6f%12.6f\n" % (   len ( self.system.energyModel.qcAtoms.QCAtomSelection ( )), self.grid.origin[0], self.grid.origin[1], self.grid.origin[2] ) )
        
        self.file.write ( "%5d%12.6f%12.6f%12.6f\n" % ( self.grid.npoints[0], self.grid.gridspacing, 0.0, 0.0 ) )
        self.file.write ( "%5d%12.6f%12.6f%12.6f\n" % ( self.grid.npoints[1], 0.0, self.grid.gridspacing, 0.0 ) )
        self.file.write ( "%5d%12.6f%12.6f%12.6f\n" % ( self.grid.npoints[2], 0.0, 0.0, self.grid.gridspacing ) )
        numbers = self.system.atoms.GetItemAttributes ( "atomicNumber" )
        for ( i, s ) in enumerate ( self.qcSelection ):
            self.file.write ( "%-5s%12.6f%12.6f%12.6f%12.6f\n" % ( numbers[s], float ( numbers[s] ), self.qcCoordinates3[i,0], self.qcCoordinates3[i,1], self.qcCoordinates3[i,2] ) )

    def WriteOrbitalHeader ( self, orbitals ):
        """Write the orbital header."""
        if orbitals is None: orbitals = range ( 0, 1 )
        if len ( orbitals ) > 0:
            self.file.write ( "%5d" % ( len ( orbitals ), ) )
            for i in orbitals: self.file.write ( "%5d" % ( i, ) )
            self.file.write ( "\n" )
